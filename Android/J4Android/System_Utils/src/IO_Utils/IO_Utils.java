...
public class IO_Utils {
    ...
    public static File writeUsingBufferedWriter(File f, LinkedList<String> lines, boolean appendMode, boolean newLine){
        ...
        if (appendMode){
            try (OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(path.toFile(), true), CHARSET);
            ...
            try (OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(path.toFile()), CHARSET);
                 BufferedWriter bw = new BufferedWriter(osw);) {
                for (String line : lines) {
                    bw.write(String.format("%s", line));
                    bw.newLine();
...
//            System.out.println("buffer.length:"+buffer.length());
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.O)
    public static void writeBytesUsingBufferedOutputStream(File f, byte[] data, boolean appendMode){
        Path path = Paths.get(f.getAbsolutePath());
        if(appendMode){
            try (BufferedInputStream bis = new BufferedInputStream(new ByteArrayInputStream(data));
                 BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(path.toFile(), true));){
                int read = -1;
