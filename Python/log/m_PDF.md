...
```
def mergePdfs_260424(directory, output_pdf_file, filepathOrder=None):
    """
    合并指定目录下的PDF文件为一个PDF文件。不带目录
...
def extract_pages_to_jpg_260423(input_pdf_path, output_dir, page_numbers_str, dpi=300):
    """
    ...
    if page_numbers_str is None:
        ...
        pdf2images_260423(input_pdf_path, dpi, "jpg", output_dir)
    else:
...
def pdf2images_260423(pdfFile, dpi=200, format='png', output_directory="2images"):
    """
    拆分PDF到图片
    Args:
        pdfFile: PDF路径
